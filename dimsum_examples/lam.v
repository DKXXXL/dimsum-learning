...
Lemma subst_map_empty e :
  subst_map ∅ e = e.
Proof.
  induction e as [| |??? IHe1 IHe2|? IHe1|? IHe1|?? IHe1 IHe2|??? IHe1 IHe2 IHe3|??? IHe1 IHe2|??? IHe1|?? IHe1 IHe2|? IHe1|? IHe1|].
  all: simpl; try rewrite IHe1; try rewrite IHe2; try rewrite IHe3; auto.
  - by rewrite delete_empty IHe2.
  - rewrite delete_empty. f_equal. 
    induction args; simpl; auto.
    rewrite delete_empty; auto.
  - f_equal. induction args.
    -- reflexivity.
    -- simpl. inversion IHe2; subst. f_equal; [done|naive_solver]. 
Qed.
...
Proof.
  ...
  induction e as [| |??? IHe1 IHe2|? IHe1|? IHe1|?? IHe1 IHe2|??? IHe1 IHe2 IHe3|??? IHe1 IHe2|a ?? IHe1|?? IHe1 IHe2|? IHe1|? IHe1|] => m/=.
  all: try rewrite IHe1; try rewrite IHe2; try rewrite IHe3; auto.
  - case_bool_decide; by simplify_map_eq.
  - case_bool_decide; f_equal. 
    + rewrite delete_insert_ne; [by rewrite IHe2|done].
    + subst; by rewrite delete_insert_delete.
  - f_equal. case_bool_decide as H.
    + f_equal. apply elem_of_cons in H. destruct!.
      -- by rewrite delete_insert_delete.
      -- destruct (decide (x = a)) eqn:?; subst; [by rewrite delete_insert_delete|].
         rewrite delete_insert_ne; [|done].
         generalize dependent m. induction args as [|hd tl IH]; [by apply elem_of_nil in H|simpl].
         apply elem_of_cons in H. destruct! => *; [by rewrite delete_insert_delete|].
         destruct (decide(x = hd)) eqn:?; subst; [by rewrite delete_insert_delete|].
         rewrite delete_commute delete_insert_ne; [|done]. rewrite delete_commute. naive_solver.
    + apply not_elem_of_cons in H. destruct H as [H1 H2]. 
      generalize dependent m. induction args as [|hd tl IH] => /=*; [by rewrite delete_insert_ne|].
      apply not_elem_of_cons in H2; destruct!. rewrite delete_commute delete_insert_ne; [|done].
      rewrite delete_commute. naive_solver. 
  - f_equal. induction args; [done|]. simpl. inversion IHe2; subst. f_equal; [done|naive_solver].
Qed.
...
Proof.
  elim: e; try naive_solver; simpl.
  - move => *. case_bool_decide; [apply is_static_expr_value|done]. 
  - move => *. case_bool_decide; naive_solver.
  - move => *. case_bool_decide; naive_solver.  
  - move => ? args ? HForall H. apply andb_prop_intro; split!; [naive_solver|]. 
    induction args as [|hd tl IH]; [done|simpl]. inversion HForall. apply andb_prop_intro. split!. 
    + apply andb_prop_elim in H. destruct!/=. naive_solver.
    + subst. apply IH; [done|]. apply andb_prop_elim in H. apply andb_prop_intro. destruct!/=. split!; done.  
Qed.
...
Lemma lookup_disjoint_none_left m1 m2 key payload: 
  ...
  key ∈ get_string_set_from_fid_set m2 →
  m1 !! (key,payload) = None.
Proof.
  move => H H0.  
  symmetry in H.
  remember (not_elem_of_disjoint _ _ _ H0 H) as n. 
  destruct (m1!!(key,payload)) eqn:K; auto.
  assert (key ∈ ((set_map (λ f : fid, f.1) (dom m1)):gset string)) as H1.
  { rewrite elem_of_map. exists (key,payload). split!. by rewrite elem_of_dom. }
  by apply n in H1.
Qed.

Lemma lookup_disjoint_none_right m1 m2 key payload: 
  get_string_set_from_fid_set m1 ## get_string_set_from_fid_set m2 → 
  key ∈ get_string_set_from_fid_set m1 →
  m2 !! (key,payload) = None.
Proof.
  move => H H0. 
  remember (not_elem_of_disjoint _ _ _ H0 H) as n. 
  destruct (m2!!(key,payload)) eqn:K; auto.
  assert (key ∈ ((set_map (λ f : fid, f.1) (dom m2)):gset string)) as H1.
  { rewrite elem_of_map. exists (key,payload). split!. by rewrite elem_of_dom. }
  by apply n in H1.
Qed.
...
Lemma get_string_sets_disjoint_implies_sets_disjoint m1 m2:
  ...
  m1 ##ₘ m2.
Proof.
  unfold get_string_set_from_fid_set => H.
  apply map_disjoint_dom_2.
  set_solver.
Qed.

Lemma elem_left_disjoint (s1 s2 : gset string) x: s1 ## s2 → x∈s1 → x ∉ s2.
Proof.
  set_solver.
Qed.

Lemma elem_right_disjoint (s1 s2 : gset string) x: s1 ## s2 → x ∈ s2 → x ∉ s1.
Proof.
  set_solver.
Qed.

Lemma get_string_set_same_matching_fns a b: 
  (∀ f, a !! f = None ↔ b !! f = None) →
  get_string_set_from_fid_set a = get_string_set_from_fid_set b.
Proof.
  move => H.
  set_unfold => x.
  split; intros; destruct!; split!;try done; rewrite elem_of_dom;
  rewrite elem_of_dom in H2; inversion H2 as [fnd H0].
  - destruct (b !! x0) eqn:Heqo; [naive_solver|]. rewrite -H in Heqo.
    by rewrite H0 in Heqo.
  - destruct (a !! x0) eqn:Heqo; [naive_solver|]. rewrite H in Heqo.
    by rewrite H0 in Heqo.
Qed.   
...
| NewRefS n l lis h h' fns: 
  (heap_alloc_prop h h' l n) → 
  head_step (Lam (NewRef (Val $ ValNum n)) lis h fns) None 
  (λ σ', n > 0 ∧ σ' = Lam (Val $ ValLoc l) lis h' fns)
| NewRefUBS v lis h fns: 
  not_val_num v →
...
| CallUBS v args lis h fns: (* App non fid *)
  not_val_fid v →
  head_step (Lam (App (Val v) (Val <$> args)) lis h fns) None 
  (λ σ', False)
...
Proof.
  ...
  have [//|??]:= Hg _ _ _ _ _ _ ltac:(done). subst.
...
Proof.
  ...
  - intros[]; auto; simpl; [done|]. repeat case_match; [done|]. by destruct!. 
  - move => f args Hall Hargs. f_equal => H. 
    apply andb_prop_elim in H. destruct H.
    f_equal; [naive_solver|]. rewrite -list_fmap_compose.
    induction args as [|hd tl IH]; [done|simpl].
    inversion Hargs. destruct!/=. f_equal; naive_solver.
Qed.
...
Proof.
  ...
  - move => /andb_prop_elim [He Hargs]. apply andb_prop_intro; split!; [naive_solver|].
    induction args => /=; [done|].
    apply andb_prop_elim in Hargs as [Ha Hargs']. revert select (Forall _ _) => HForall.
    inversion HForall; subst. 
    apply andb_prop_intro; split!; naive_solver.
Qed.
...
Proof.
  ...
  eapply steps_spec_step_end; [econs; [done|econs]|naive_solver].
Qed.
...
Proof.
  destruct LamExprFill0; subst.
  econs => *. apply steps_impl_step_end =>  ?? /prim_step_inv_head[| |?[??]].
  { solve_sub_redexes_are_values. } { done. }
  subst. inv_all head_step. naive_solver.
Qed.
...
Proof.
  destruct LamExprFill0; subst. econs => *. split! => *; [done|]. 
  apply: steps_spec_step_end; [econs; [done|econs]|naive_solver]. 
Qed.
Global Hint Resolve lam_step_Binop_s | 10 : typeclass_instances.

(* TODO: BEAUTIFY PROOFS*)
...
Proof.
  destruct LamExprFill0; subst.
  econs => ? HG. apply: steps_impl_step_end => ?? /prim_step_inv_head[| |?[??]].
...
Lemma lam_step_Newref_s fns s h e K v  `{!LamExprFill e K (NewRef (Val v) )}:
  TStepS lam_trans (Lam e s h fns) (λ G, (G None (λ G', 
    if v is ValNum n 
      then ∃ l h', heap_alloc_prop h h' l n ∧ (n > 0 → G' (Lam (expr_fill K (Val (ValLoc l))) s h' fns))
      else True ))).   
Proof.
  destruct LamExprFill0; subst.
  econs => ? HG. split! => *; [done|].
...
Proof.
  destruct LamExprFill0; subst.
  econs. intros. apply steps_impl_step_end. intros. 
  apply prim_step_inv_head in H0.
  - destruct H0 as [?[??]]. subst.
  inversion H0; simplify_eq.  naive_solver.
  - solve_sub_redexes_are_values.
  - reflexivity. 
Qed.
...
Proof.
  ...
  eapply steps_spec_step_end; [econs; [done|econs; destruct H0; done]|naive_solver]. 
Qed. 
...
Proof.
  destruct LamExprFill0, AsVals0; subst.
  econs => ? [??]. apply steps_impl_step_end => ?? /prim_step_inv_head[| |?[Hstep ?]].
  { apply sub_redexes_are_values_item; case; try naive_solver.
    move => /= ??? e' [_ Heq]. rewrite right_id_L in Heq. by apply: list_expr_val_inv. }
  { done. }
  subst. rewrite right_id_L in Hstep. inv Hstep; naive_solver.
Qed.
...
Lemma lam_step_App_s fns s h e K v  vs `{!LamExprFill e K (App (Val v) es)} `{!AsVals es vs None}:
  TStepS lam_trans (Lam e s h fns) (λ G,
    if v is ValFid f then 
      (f.1 ∈ get_string_set_from_fid_set fns ∧ G None  
        (λ G', ∀ fn, fns !! f = Some fn → length vs = length fn.(fd_args) → 
          G' (Lam (expr_fill (ReturnIntCtx :: K) (subst_l fn.(fd_args) vs fn.(fd_body))) (f.1 :: s) h fns)))
          ∨
      (f.1 ∉ get_string_set_from_fid_set fns ∧ G (Some (Outgoing, ELCall  f vs h)) (λ G', G' (Lam (expr_fill K Waiting) s  h fns)))
      else 
      G None (λ G', True)).
Proof.
  destruct LamExprFill0, AsVals0; subst.
  econs => ? HG. case_match. 
  4: { destruct!. 
      - split!; [done|]. move => ??. rewrite right_id_L. apply: steps_spec_step_end; [econs; [done|by econs]|].
        naive_solver.
      - split!; [done|]. move => ??. rewrite right_id_L. apply: steps_spec_step_end; [econs; [done|by econs]|].
        naive_solver. }
  all: split!; [done..|]; move => *; apply: steps_spec_step_end;
      [rewrite right_id_L; econs; [done|by econs]|naive_solver].
Qed.
...
Lemma fns_match_some fns1 fns2: 
  lam_proof_fns_match fns1 fns2 → 
  (∀ f, (∃fn,fns1 !! f = Some fn )↔(∃fn, fns2 !! f = Some fn)).
Proof.
  intros.
  unfold lam_proof_fns_match in H.
  split; intros; destruct!.
  - destruct (fns2 !! f) eqn:?; [naive_solver|].
  rewrite - H in Heqo.
  rewrite H0 in Heqo.
  inversion Heqo.
  - destruct (fns1 !! f) eqn:?; [naive_solver|].
  rewrite H in Heqo.
  rewrite H0 in Heqo.
  inversion Heqo.
Qed.

Definition lam_proof_call (n : ordinal) P :=
  (∀ n' fns1 fns2 f es1' es2' K1' K2' s1 s2 es1 es2 vs1' vs2' h1' h2' b,
      P fns1 fns2 →
      ...
      (∀ v'' h'' fnsa fnsb,
          ...
          Lam (expr_fill K1' (Val v'')) s1  h'' fnsa
              ...
              Lam (expr_fill K2' (Val v'')) s2 h'' fnsb) →
      Lam es1' s1 h1' fns1
          ⪯{lam_trans, lam_trans, n', b}
      Lam es2' s2 h2' fns2).

Lemma lam_proof fns1 fns2 (P:_ → _ → Prop): 
  (∀ a b, P a b →lam_proof_fns_match a b) →
  (P fns1 fns2) →
  (∀ n K1 K2 s1 s2 fns1' fns2' f fn1 vs h,
  P fns1' fns2' →
  fns1' !! f = Some fn1 →
...
Proof.
  ...
  tstep_both. split!.
  2:{intros. tstep_s. right. eexists _,_. split. 
  - destruct st_stack1 eqn:?; [|naive_solver].
  destruct!. destruct H7. assert ([]=[]) as ref by auto. apply H6 in ref. inversion ref.
  - split!; [auto|tend]. apply: tsim_mono_b. naive_solver. }
  intros. tstep_s. left.
  (*assert (f.1 ∈((get_string_set_from_fid_set fns2):gset string)).
  rewrite elem_of_map in H4. destruct!. 
  destruct (fns2!!x) eqn:?. rewrite elem_of_map. exists x. rewrite elem_of_dom. split!.
  rewrite -H in Heqo. rewrite elem_of_dom in H8. rewrite Heqo in H8. destruct H8. inversion H4.
  done.*) 
  split!; [|done|].
  { erewrite <- get_string_set_same_matching_fns; naive_solver. }
  tend. tstep_s. split!.
  { erewrite <- get_string_set_same_matching_fns; naive_solver. }
  intros.
  tstep_i.
  split!. intros.
  assert (∃ fn, st_fns0!! f = Some fn).
  {
    erewrite fns_match_some; [exists fn; naive_solver|]. unfold lam_proof_fns_match.
    naive_solver.
  }
  destruct!.
  assert (length vs = length (fd_args fn0)).
  {
    eapply H1 in H5; [|done]. destruct!. rewrite H12 in H6. inversion H6. subst.
    rewrite H5. naive_solver.
  }
  split!; [done..|].
  ...
  apply H3; try done. split!; try done. }
  ...
  - unfold lam_proof_call.
    move => n1 fnsa fnsb f' ?? Ka Kb ?? es1 es2 vs1' vs2' ??? Pcond [?][?] ? Hall1 Hall2 ????.
    ...
    + intros. tstep_s. left. split!. 
      ...
      intros. tend. 
      assert (∃ fn, fnsa!! f' = Some fn). {
        erewrite <-fns_match_some; [exists fn2; naive_solver|]. unfold lam_proof_fns_match.
        naive_solver.
      }
      destruct!.
      assert (length vs2' = length (fd_args fn3)).
      {
        eapply H1 in Pcond; [|done]. destruct!.
...
Proof.
  ...
  - (*left case*) tstep_both. apply: steps_impl_step_end => ?? /prim_step_inv[//|?[?[?[?[??]]]]].
    ...
    + (* fixe*) 
      ...
      inversion H1; subst.
      --(* returns to None*)
        tstep_s. split!. rewrite lookup_union_None in H8. destruct!. exact H0.
        intros. tend. split!. apply Hloop. rewrite !expr_fill_app.
        ...
        split!. 3,4: rewrite H12 H11; reflexivity.
    ...
    + (* var*) tstep_s. auto. 
    + (* internal app*) 
      unfold fns_inv in H; destruct!. rewrite elem_get_string_set_union in H8. 
      tstep_s. destruct!.
      -- (* in left case *) 
        left. split!. intros. tend. split!. rewrite lookup_union_l. exact H0.
        eapply lookup_disjoint_none_right. exact H. exact H8. auto.
        apply Hloop. assert (∀K e, expr_fill K (ReturnInt e) = expr_fill (ReturnIntCtx::K) e) as H11 by auto. 
        split!.
        2,3: rewrite H11; reflexivity. apply LLCELeftToLeft. exact H1. by rewrite H2 in H8.
        auto. auto. apply is_static_expr_subst_l. apply fn.(fd_static).
      -- (* in right case *) 
        right. split!. by eapply elem_left_disjoint. repeat case_bool_decide; try auto.
        ++  exfalso. rewrite -H2 in H0. eapply elem_left_disjoint. exact H. exact H0. auto.
        ++ repeat case_bool_decide; try done. rewrite H4 in H8. rewrite elem_of_disjoint in Hdisj.
          exfalso. naive_solver.
          tstep_s. split!. tstep_s. split! => *. tend. split!.
           rewrite lookup_union_r; [done|by eapply lookup_disjoint_none_left].
           { done. }  apply Hloop. 
           assert (∀K e, expr_fill K (ReturnInt e) = expr_fill (ReturnIntCtx::K) e) as H11 by auto.
           assert (∀K e, expr_fill K (ReturnExt e) = expr_fill (ReturnExtCtx::K) e) as H12 by auto.
           split!.
           all :try rewrite H12 ;try rewrite H11; try reflexivity; try auto. by apply LLCELeftToRight. 
           apply is_static_expr_subst_l. apply (fd_static).
           by rewrite H4 in H8.
    + (* external app*) 
      unfold fns_inv in H; destruct!.
      tstep_s. right.  split!; repeat case_bool_decide. 
      rewrite elem_get_string_set_union in H8. move => ?. 
      assert (f.1 ∈ get_string_set_from_fid_set fnsl ∨ f.1 ∈ get_string_set_from_fid_set fnsr).
      by left. done. 
      rewrite elem_get_string_set_union in H8. 
      assert (f.1 ∈ get_string_set_from_fid_set fnsl ∨ f.1 ∈ get_string_set_from_fid_set fnsr).
      left. rewrite H2; done. done.
      done. done.
      rewrite elem_get_string_set_union in H8.
      assert (f.1 ∈ get_string_set_from_fid_set fnsl ∨ f.1 ∈ get_string_set_from_fid_set fnsr).
      ...
      split!. auto. 
      tend. split!. apply Hloop.  split!; try done. apply LLCELeftToNone. exact H1. auto.
    + (* app UB *)
      destruct v eqn:?. 
      4: done.
      all: tstep_s; split!. 
  - (*right case*) tstep_both. apply: steps_impl_step_end => ?? /prim_step_inv[//|?[?[?[?[??]]]]].
    simplify_eq. revert select (Is_true (is_static_expr _ _)) => /is_static_expr_expr_fill/=[??]//.
    rewrite -expr_fill_app.
    inv_all head_step => //.
    + (* binop*) 
    ...
    + (* fixe*) 
      ...
      inversion H1; subst.
      --(* returns to None*)
        tstep_s. split!. rewrite lookup_union_None in H8. destruct!. exact H3.
        intros. tend. split!. apply Hloop. rewrite !expr_fill_app.
        assert (∀K e, expr_fill K (ReturnInt e) = expr_fill (ReturnIntCtx::K) e) as H11 by auto.
        assert (∀K e, expr_fill K (ReturnExt e) = expr_fill (ReturnExtCtx::K) e) as H12 by auto.
        split!. 3,5: rewrite H12 H11; reflexivity.
    ...
    + (* internal app*) 
      ...
      tstep_s. destruct!.
      -- (* in left case *) 
        right. split!. by eapply elem_right_disjoint. repeat case_bool_decide; try auto.
        rewrite -H2 in H0.
        exfalso. eapply elem_left_disjoint. exact H. exact H8. rewrite H4. done. 
        repeat case_bool_decide.
        ++ split!. 
           tstep_s. left. split!. tstep_s. left. split!. intros. tend. split!.
           rewrite lookup_union_l. exact H3. eapply lookup_disjoint_none_right. exact H. exact H8.
           auto. apply Hloop. 
           assert (∀K e, expr_fill K (ReturnInt e) = expr_fill (ReturnIntCtx::K) e) as H11 by auto.
           assert (∀K e, expr_fill K (ReturnExt e) = expr_fill (ReturnExtCtx::K) e) as H12 by auto.
           split!.
        ...
        ++  exfalso. rewrite H2 in H8. auto.
        ++ rewrite H2 in H8. contradiction. 
      -- (* in right case *) 
        left. split!. intros. tend. split!. rewrite lookup_union_r. exact H0.
        eapply lookup_disjoint_none_left. exact H. exact H8. auto.
        apply Hloop. assert (∀K e, expr_fill K (ReturnInt e) = expr_fill (ReturnIntCtx::K) e) as H11 by auto. 
        split!.
        2,4: rewrite H11; reflexivity. apply LLCERightToRight. exact H1. by rewrite H4 in H8.
        auto. auto. apply is_static_expr_subst_l. apply fn.(fd_static).  
    + (* external app*) 
      unfold fns_inv in H; destruct!.
      tstep_s. right.  split!; repeat case_bool_decide. rewrite elem_get_string_set_union in H8 => ?.
      assert (f.1 ∈ get_string_set_from_fid_set fnsl ∨ f.1 ∈ get_string_set_from_fid_set fnsr).
      by right. contradiction.
      done. 
      rewrite elem_get_string_set_union in H8.
      assert (f.1 ∈ get_string_set_from_fid_set fnsl ∨ f.1 ∈ get_string_set_from_fid_set fnsr).
      right. by rewrite H4. done. 
      done.
      rewrite elem_get_string_set_union in H8.
      assert (f.1 ∈ get_string_set_from_fid_set fnsl ∨ f.1 ∈ get_string_set_from_fid_set fnsr).
      left. by rewrite H2. done.
      rewrite elem_get_string_set_union in H8.
      assert (f.1 ∈ get_string_set_from_fid_set fnsl ∨ f.1 ∈ get_string_set_from_fid_set fnsr).
      right. by rewrite H4. done.
      split!. auto.  
      tend. split!. apply Hloop.  split!; try done. apply LLCERightToNone. exact H1. auto.
    + (* appUB *)
      destruct v eqn:?.
      4: done.
      all: by tstep_s.                                                      
...
Proof.
  ...
  - (*left case*)
    ...
    + (* binop*) 
      ...
      apply is_static_expr_expr_fill; split!; repeat case_match;auto.
    + (* newref *)
      tstep_s. split! => *; [done|]. tend. split!. apply: Hloop; [auto|].
      rewrite !expr_fill_app. split!; [done..|].
      apply is_static_expr_expr_fill; split!. 
    + (* newrefUB *)
      tstep_s. case_match; done.
    + (* load*)
    ...
    + (* internal app*) 
      ...
      -- (* in right case *) 
        right. split!. by eapply elem_left_disjoint.  repeat case_bool_decide; try auto.
        rewrite elem_of_disjoint in Hdisj. exfalso. rewrite H4 in H7; naive_solver.
        repeat case_bool_decide.
        ++  exfalso. rewrite -H2 in H0. eapply elem_left_disjoint. exact H. exact H0. auto.
        ++ tstep_s. left. split!. tstep_s. left. split!. intros. tend. split!.
  ...
  - (*right case*) 
    ...
    + (* internal app*) 
      ...
      tstep_s. destruct!.
      -- (* in left case *) 
        right. split!. repeat case_bool_decide; try auto. by eapply elem_left_disjoint. 
        repeat case_bool_decide.
        done.
        by rewrite H2 in H7. done.
        repeat case_bool_decide.
        ++ tstep_s. left. split!. tstep_s. left. split!. intros. tend. split!.
    ...
    + (* external app*) 
      unfold fns_inv in H; destruct!.
      tstep_s. right.  split!; repeat case_bool_decide. intro.
      rewrite elem_get_string_set_union in H7. 
      assert (f.1 ∈ get_string_set_from_fid_set fnsl ∨ f.1 ∈ get_string_set_from_fid_set fnsr).
      by right. contradiction.
      done. 
      rewrite elem_get_string_set_union in H7.
      assert (f.1 ∈ get_string_set_from_fid_set fnsl ∨ f.1 ∈ get_string_set_from_fid_set fnsr).
      right. by rewrite H4. contradiction.
      done.
      rewrite elem_get_string_set_union in H7.
      assert (f.1 ∈ get_string_set_from_fid_set fnsl ∨ f.1 ∈ get_string_set_from_fid_set fnsr).
      left. by rewrite H2. contradiction.
      rewrite elem_get_string_set_union in H7.
      assert (f.1 ∈ get_string_set_from_fid_set fnsl ∨ f.1 ∈ get_string_set_from_fid_set fnsr).
      right. by rewrite H4. contradiction.
      split!. auto. 
...
Proof.
  ...
  - (* ** MLFLeft case*)
    ...
    + tstep_both. apply: steps_impl_step_end => ?? /prim_step_inv[//|?[?[?[?[??]]]]] *.
      ...
      * (*newref*) 
        tstep_s. split!; [done|]. move => *.
        tend. split!. rewrite !expr_fill_app. apply: Hloop; [done|].
        split!; [done..|]. apply is_static_expr_expr_fill. split!.
      * (*newrefUB*) tstep_s. case_match; done. 
      ...
      * (* fix e*) (*need to assert that s1 i nonempty*)
        ...
        -- (* case 3 where it returns to itself*)
          ...
          ** apply is_static_expr_expr_fill. split!.
      * (* var*) by tstep_s.
      * (* app internal in fnsl*) tstep_s => *.  left. split!. unfold fns_inv in *.
      ...
      *  move => *. destruct!/=. repeat case_bool_decide => //.
        ...
        -- (* app external*) tstep_s. right. split!. unfold fns_inv in H. destruct! => H'. rewrite elem_get_string_set_union in H'. 
            destruct!. by rewrite H10 in H'. done. 
  ...
  - (* ** MLRight case*)
    ...
    + tstep_both. apply: steps_impl_step_end => ?? /prim_step_inv[//|?[?[?[?[??]]]]] *.
      ...
      *  move => *. destruct!/=. repeat case_bool_decide => //.
        ...
        destruct!; simpl; auto. auto. apply is_static_expr_subst_l. apply fn.(fd_static). 
        -- (* app external*) tstep_s. right. split!. unfold fns_inv in H. destruct!.
            move => H'. rewrite elem_get_string_set_union in H'. destruct!.
            by rewrite H6 in H'. done.
            tend. split!. apply Hloop; try done. split!; try done. econs. exact H1. done.
      * (* app UB *) 
        destruct v eqn:?.
        4: done.
