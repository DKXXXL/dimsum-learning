...
From dimsum.core Require Export trace_index.
...
Record min_ord := MinOrd { min_ord_car : trace_index }.
...
Section min_ord.
  Local Instance min_ord_unit_instance : Unit min_ord := MinOrd (tiChoice Empty_set (λ x, match x with end)) .
  ...
  Local Instance min_ord_op_instance : Op min_ord := λ n m, MinOrd (ti_min (min_ord_car n) (min_ord_car m)).
  Definition min_ord_op_min x y : MinOrd x ⋅ MinOrd y = MinOrd (ti_min x y) := eq_refl.

  Lemma min_ord_included (x y : min_ord) : x ≼ y ↔ min_ord_car y ⊆ min_ord_car x.
  Proof.
    split.
    - intros [z [-> ?]]. simpl. apply ti_min_le_l.
    - exists y. rewrite /op /min_ord_op_instance. symmetry. by apply ti_min_r.
  Qed.
  ...
  Proof.
    ...
    - intros [x] [y] [z]. repeat rewrite min_ord_op_min. by apply ti_min_assoc.
    - intros [x] [y]. rewrite !min_ord_op_min. apply ti_min_comm.
    - intros [x]. rewrite min_ord_op_min. apply ti_min_id.
  Qed.
  ...
  Proof.
    ...
    split => /=.
    - apply ti_min_l_2. rewrite Hle. apply ti_min_le_r.
    - apply ti_min_le_l.
  Qed.

  Global Instance : LeftAbsorb (≡) (MinOrd tiO) (⋅).
  Proof. move => [x]. rewrite min_ord_op_min. apply ti_min_l. constructor. Qed.

  Global Instance : RightAbsorb (≡) (MinOrd tiO) (⋅).
  Proof. move => [x]. rewrite min_ord_op_min. apply ti_min_r. constructor. Qed.

  Global Instance : IdemP (≡@{min_ord}) (⋅).
  Proof. intros [x]. rewrite min_ord_op_min. apply ti_min_id. Qed.

  Global Instance min_ord_is_op x y :
    IsOp (MinOrd (ti_min x y)) (MinOrd x) (MinOrd y).
  Proof. done. Qed.
End min_ord.
...
update. *)
Definition mono_ord_auth (q : Qp) (n : trace_index) : mono_ord :=
  ●{#q} Some (MinOrd n) ⋅ ◯ Some (MinOrd n).
Definition mono_ord_ub (n : trace_index) : mono_ord := ◯ Some (MinOrd n).
...
Section mono_ord.
  ...
  Lemma mono_ord_ub_op n1 n2 :
    mono_ord_ub n1 ⋅ mono_ord_ub n2 = mono_ord_ub (ti_min n1 n2).
  ...
  Proof.
    ...
    do 3 f_equiv. symmetry. apply ti_min_id.
  Qed.
  ...
  Proof. intros. rewrite mono_ord_ub_op /mono_ord_ub. do 2 f_equiv. symmetry. by apply ti_min_r. Qed.
  ...
  Proof.
    ...
    split.
    - move => [?[??]]. split; [done|]. etrans; [done|]. apply ti_min_le_r.
    - move => [??]. split; [done|]. split; [|done]. by apply ti_min_l_2.
  Qed.
